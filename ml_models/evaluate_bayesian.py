...
class BayesianModelEvaluator:
    ...
    def __init__(self, model_path: str):
        ...
        """
        self.predictor = BayesianRacePredictor(model_path)
    ...
    def _evaluate_prediction(
        ...
        """予測を評価"""
        recommended_boats = prediction_df.head(3)['boat_number'].tolist()
        ...
        # 不確実性の評価（ベイズ特有）
        avg_std = prediction_df.head(3)['std'].mean()
    ...
    def _aggregate_results(self, results: list) -> dict:
        ...
        print()
        print("ベイズ特有の指標:")
        print(f"  平均不確実性: {summary['avg_uncertainty']:.3f}")
        print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="階層ベイズモデル評価")
    ...
    parser.add_argument("--all-grades", action="store_true", help="全グレードを対象")
    ...
    evaluator = BayesianModelEvaluator(args.model)
